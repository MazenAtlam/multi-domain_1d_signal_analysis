...
export async function resampleECG(file, targetFmax, mode, originalSr) {
    ...
    // Send parameters - try different parameter names that the backend might expect
    formData.append('target_sr', targetSr.toString());
    formData.append('fs', originalSr.toString()); // Try 'fs' which is common for sampling rate
    formData.append('original_sr', originalSr.toString());
    formData.append('mode', mode);
    // Also send the fmax in case backend needs it
    formData.append('fmax', targetFmax.toString());

    console.log(`Resampling parameters: target_sr=${targetSr}, original_sr=${originalSr}, mode=${mode}, fmax=${targetFmax}`);
    ...
    if (!response.ok) {
        ...
        if (errorBody.includes('sampling rate') || errorBody.includes('Could not determine')) {
            throw new Error(`Sampling rate detection failed. Please ensure your file contains time information or specify the sampling rate. Original error: ${errorBody}`);
        } else {
            throw new Error(`Resampling failed: ${errorBody}`);
        }
    }
